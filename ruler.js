...
.factory('rvRule', function () {
  ...
  var Basic = function (params, data) {
    var a = Rule.pluck(params.path, data);
    var b = params.match;
  };
  ...
  Regex.prototype.match = function () {
    ...
    var result = comparisonValue.match(
      new RegExp(this.params.pattern, this.params.flags));
    ...
    */
    exec: function (data, pass, fail) {
      ...
      for(var i = 0; i < this.rules.length; i++) {
        ...
        this.rules = [];
        var result = new rvRule[filter.type](filter.params, data)[filter.params.comparison]();
        if(result === false) {
          fail(); // callback
          return false;
        }
